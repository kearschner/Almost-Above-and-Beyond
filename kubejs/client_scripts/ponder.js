onEvent("ponder.registry", event => {
    ...
    event.create("kubejs:alchemical_laser")
        .scene("alchemy_setup", "The Thermal Alchemy Setup", "kubejs:laser_alchemy", (scene, util) => {
            ...
            scene.text(50, "Start with an Invar Machine", util.vector.topOf(2,1,3)).colored(PonderPalette.WHITE).placeNearTarget();
            scene.idle(60)

            scene.text(50, "Attach a Rose Quartz Lamp", util.vector.centerOf(2,1,2)).colored(PonderPalette.WHITE).placeNearTarget();
            scene.idle(30)

            scene.world.showSection(light, Facing.NORTH)
            scene.idle(25)

            scene.world.modifyBlock([2,1,2], (curState) => curState.with("powering", "true"), false)
            scene.showControls(50, util.vector.blockSurface(util.grid.at(2,1,2), Facing.east), "right").rightClick().withWrench()
            scene.world.toggleRedstonePower(light)
            scene.effects.indicateRedstone(util.grid.at(1, 1, 2))
            //scene.world.setBlock(util.grid.at(2, 1, 2), util.getDefaultState("create:rose_quartz_lamp"), false)
            scene.idle(15)

            scene.text(40, "Ensure that the light is powered", util.vector.centerOf(2,1,2)).colored(PonderPalette.GREEN)
            scene.idle(50)
            ...
            scene.text(60, "Place a Deployer targeting the Invar Machine", util.vector.topOf(4,1,3)).colored(PonderPalette.WHITE).placeNearTarget()
            scene.idle(70)

            scene.showControls(50, util.vector.blockSurface(util.grid.at(4,1,3), Facing.west), "left").rightClick().withWrench()
            scene.idle(8)
            scene.world.modifyTileNBT(deployerSingle, java("com.simibubi.create.content.contraptions.components.deployer.DeployerTileEntity"), (nbt) => {
                nbt.Mode = "PUNCH";
            });

            scene.text(50, "Set it to Punch mode using a Wrench", util.vector.topOf(4,1,3)).colored(PonderPalette.GREEN).placeNearTarget()
            scene.idle(20)
            ...
            scene.text(50,"Whenever the Deployer activates...",util.vector.topOf(4, 1, 3)).colored(PonderPalette.WHITE).placeNearTarget().attachKeyFrame()
            scene.idle(30)
            ...
            scene.text(50,"...the Laser Lamp will emit a High-Energy Beam", util.vector.centerOf(2,1,2)).colored(PonderPalette.GREEN).placeNearTarget()
            scene.idle(60)
            ...
            scene.text(80, "Hopper Minecarts in the Beam will process thier contained Items", util.vector.centerOf(2,1,0)).colored(PonderPalette.WHITE).placeNearTarget()
            scene.idle(70)

            scene.showControls(40, util.vector.centerOf(2, 1, 0), "down").withItem("thermal:flux_magnet")
            scene.idle(5)

            scene.showControls(35,util.vector.centerOf(2, 1, 0),"up").withItem("minecraft:basalt")
            scene.idle(30)
            ...
            scene.showControls(35, util.vector.centerOf(2,1,0), "up").withItem("thermal:basalz_rod")
